...
Options:
      ...
      --from-registry-username USER                       Set the username to use when pulling from registry, e.g. gitlab-ci-token.
      --from-registry-password PASSWORD                   Set the password to use when pulling from registry, e.g. ${CI_JOB_TOKEN}.
